...
class _TaskDetailsState extends State<TaskDetails> {
  ...
  Widget build(BuildContext context) {
    final taskProv = context.read<TaskProviderClass>();
        ...
        decoration: BoxDecoration(
          ...
          child:  Padding(
            ...
            child: SingleChildScrollView(
              ...
              child: Consumer<TaskProviderClass>(builder: (context, provider, child){
                final Task updatetask = provider.allTask.firstWhere(
                      (t) => t.docid ==widget.task.docid,
                  orElse: () => widget.task,
                );
                ...
                return Column(
                      ...
                      Row(
                        children: [
                          Text(updatetask.title, style: TextStyle(
                            color: Colors.white,
                      ...
                      await showModalBottomSheet(
                           context: context,
                           builder: (context){
                             return BottomSheetClass(task: updatetask,);
                           }
                           );
                           if (updated == true) {
                      ...
                      Row(
                        children: [
                          ...
                          Text(updatetask.date, style: TextStyle(color: Colors.white),),
                          Text("  |  ", style: TextStyle(color: Colors.white),),
                          Icon(Icons.access_time, color: Colors.white, size: 15,),SizedBox(width: 3,),
                          Text(updatetask.time, style: TextStyle(color: Colors.white),),
                        ],
                      ),
                        SizedBox(height: 30,),
                        ...
                        children: [
                          Expanded(
                            child: Padding(
                              ...
                              child: Text(updatetask.description,
                                style: TextStyle(
                                color: Colors.white
                              ),),
                            ),
                            ...
                            SizedBox( height: 61, width: 78,
                              ...
                              boxShadow: [ BoxShadow(
                                ...
                                child: ElevatedButton(onPressed:()async {
                                  try {
                                    await provider.doneTask(updatetask.docid);
                                    Navigator.pop(context);
                                  }catch (e) {
                                    ScaffoldMessenger.of(context).showSnackBar(
                                      SnackBar(
                                   ...
                                   return AlertDialog(
                                     ...
                                     actions: [
                                       ElevatedButton(
                                           onPressed: () async{
                                             try {
                                            await provider.deleteTask(updatetask.docid);
                                             Navigator.pop(context);
                                             Navigator.pop(context);
                                             }catch (e) {
                                               ScaffoldMessenger.of(context).showSnackBar(
